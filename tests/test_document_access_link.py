...
class DocumentAccessLinkTests(unittest.TestCase):
    ...
    def test_corrupt_document_is_recorded_without_blocking_case_flow(self) -> None:
        ...
        # Contract (Deliverable 18): unsupported type stays 415 — but the
        # attempt is recorded (queryable) and the analyst is notified (W3-07).
        self.assertEqual(response.status_code, 415)
        body = response.json()["detail"]
        self.assertEqual(body["error"], "file_type_not_allowed")
